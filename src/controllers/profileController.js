...
class ProfileController {
  ...
  static async postProfile(req, res) {
    ...
    if (!data.user_name || !data.user_email) {
      return res.redirect(
        "/profile?msgError=Campos nome e Email prescisam ser preenchidos"
      );
    }
    ...
    const updateUser = {
      ...
      user_curso_interesse: data.user_curso_interese,
    };
    ...
    if (!result) {
      return res.redirect(
        "/profile?msgError=Não foi possivel atualizar o perfil de usuario!"
      );
    }

    const checkExists = await RegisterUselModel.getUserById(getId);
